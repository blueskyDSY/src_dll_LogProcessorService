    ...
    public class MyLogManager
    {
        ...
        private void DeleteLogFileByDateRule()
        {
            ...
            try
            {
                ...
                if (!System.IO.Directory.Exists(m_logPath))  //log目录不存在
                {
                    Log.Kernel.LogDebug("DeleteLogFileByDateRule LogDir " + m_logPath + " not Exists");
                    return;
                }
        ...
        private void DeleteLogFileBySize()
        {
            ...
            try
            {
                if (!System.IO.Directory.Exists(m_logPath))  //log目录不存在
                {
                    Log.Kernel.LogDebug("DeleteLogFileBySize LogDir " + m_logPath+ " not Exists");
                    return;
                }
        ...
        private void CompressLogFiles()
        {
            ...
            try
            {
                ...
                DateTime baseLevel = level.Subtract(new TimeSpan(5, 0, 0, 0, 0));  //创建时间超过5天的文件，压缩
                DateTime createTime;
                ...
                foreach (var item in m_dicNeedDelFiles)
                {
                    ...
                    string outputFile = string.Format(@"log\Compress{0:D4}{1:D2}{2:D2}.zip", item.Key.Year, item.Key.Month, item.Key.Day);
        ...
        #region field
        protected static MyLogManager s_logManager;
